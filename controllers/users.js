...
module.exports.renderSignupForm =  (req, res) => {
    const defaultRole = req.query.role === 'owner' ? 'owner' : 'user';
...
module.exports.signup = async (req, res) => {
    try {
        ...
        req.session.pendingRegistration = { username, email, password, role, otp, expiresAt };
...
module.exports.verifyOtp = async (req, res) => {
    try {
        ...
        const { username, email, password, role } = pending;
        ...
        try {
            ...
            if (req.user.role !== chosenRole) {
                ...
                req.logout(function(err){
                    if(err) console.error('Logout error after role mismatch:', err);
                });
                req.flash('error', `You are not registered as '${chosenRole}'. Please use the correct login option.`);
                return res.redirect('/login');
            }
            req.flash("success", `Welcome back to BOOK&STAY — logged in as ${req.user.role}`);
